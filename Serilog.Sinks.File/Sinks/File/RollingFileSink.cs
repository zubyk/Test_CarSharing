...
sealed class RollingFileSink : ILogEventSink, IFlushableFileSink, IDisposable
{
    ...
    readonly int _rollingIntervalDuration;
                          ...
                          int rollingIntervalDuration)
    {
        ...
        if (rollingIntervalDuration is < 1) throw new ArgumentException("Zero or negative value provided; rollingIntervalDuration count limit must be at least 1.");
        ...
        _rollingIntervalDuration = rollingIntervalDuration;
        _hooks = hooks;
    }
    ...
    void OpenFile(DateTime now, int? minSequence = null)
    {
        ...
        _nextCheckpoint = _roller.GetNextCheckpoint(now, _rollingIntervalDuration) ?? now.AddMinutes(30);
