...
public static class FileLoggerConfigurationExtensions
{
        ...
        TimeSpan? flushToDiskInterval)
    {
        return File(sinkConfiguration, path, restrictedToMinimumLevel, outputTemplate, formatProvider, fileSizeLimitBytes,
            levelSwitch, buffered, shared, flushToDiskInterval, RollingInterval.Infinite, false, null, null, null, null);
    }
        ...
        TimeSpan? flushToDiskInterval)
    {
        return File(sinkConfiguration, formatter, path, restrictedToMinimumLevel, fileSizeLimitBytes, levelSwitch,
            buffered, shared, flushToDiskInterval, RollingInterval.Infinite, false, null, null, null, null);
    }
    ...
    /// <param name="rollingIntervalDuration">Duration of rolling interval period</param>
    /// <returns>Configuration object allowing method chaining.</returns>
    [Obsolete("New code should not be compiled against this obsolete overload"), EditorBrowsable(EditorBrowsableState.Never)]
    public static LoggerConfiguration File(
        this LoggerSinkConfiguration sinkConfiguration,
        ...
        int? rollingIntervalDuration = null)
    {
        return File(sinkConfiguration, path, restrictedToMinimumLevel, outputTemplate, formatProvider, fileSizeLimitBytes, levelSwitch, buffered,
            shared, flushToDiskInterval, rollingInterval, rollOnFileSizeLimit, retainedFileCountLimit, encoding, rollingIntervalDuration);
    }
        ...
        FileLifecycleHooks? hooks = null)
    {
        ...
        return ConfigureFile(sinkConfiguration.Sink, formatter, path, restrictedToMinimumLevel, null, levelSwitch, false, true,
            false, null, encoding, RollingInterval.Infinite, false, null, hooks, null, 1);
    }
        ...
        int? rollingIntervalDuration)
    {
        ...
        rollingIntervalDuration = Math.Max(rollingIntervalDuration ?? 1, 1);
        ...
        try
        {
            ...
            {
                sink = new RollingFileSink(path, formatter, fileSizeLimitBytes, retainedFileCountLimit, encoding, buffered, shared, rollingInterval, rollOnFileSizeLimit, hooks, retainedFileTimeLimit, rollingIntervalDuration.Value);
            }
            else
            {
                if (shared)
